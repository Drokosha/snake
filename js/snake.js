...
function snakeObject() {
    ...
    var i = 1; //задаю координату х элементов массива snake.
    var p = 1; //переменная которая считает элементы массива snake 
    var line = 0; //задаю координату Y элементов массива, так же она она определяет в какой строке canvas мы находимся.

    snake[0] = new SnakePart({x: 0, y: 0}, {x: 1, y: 0}, null, ctx); // 1 элемент массива в котором присваиваем объекты класса SnakePart, в котором лежит хвост. 

    for (p; p <= widthH - 2; p++) { // Создаем цикл, в котором путем перебора создаем элементы массива snake и задаем им координаты. 
                snake[p] = new SnakePart({x: i, y: line}, {x: i + 1, y: line}, {x: i - 1, y: line}, ctx); // вычесление координат элементов массива snake, в которых находятся объекты класса SnakePart
                i++; //каждый раз после создания элемента массива snake, увеличиваем координату X, для следующего элемента массива snake
            } //Соотвественно задаем координаты coord prev next в соотвествии с конструктором

    snake[p] = new SnakePart({x: p, y: line}, {x: p, y:line + 1}, {x: p - 1, y: line}, ctx); // создаем элемент массива, который отвечает за 1 поворот в строчке 
    ...
    do {
        ...
        line++; // после того как мы создали элементы массива в 0 строке, мы должны увеличить значение Y (количество строк) на 1
        // так как у нас элементы массива будут создаваться по порядку или  слева направо, или справа на лево, то  нам необходимо определить четная у нас строка или нет
        ...
        if (line % 2 != 0) { // проверка на четность, если line (строка) не четная то заходим в оператора и производит действия
            ...
            i = 29; //Так как у нас координаты пойдут справа на лево и последняя клетка в строке имеет данное значение, это и будет 1 координата 1 элемента массива во второй строке. 
            ...
            i --; // уменьшаю координату Х, так как при вхождении в цикл нам нужно следующая ячейка

            for (var com = widthH - 2; com >= 1; com--) {// собственно цикл создания элементов массива snake с 28 до 1
    ...
    } while (line <= 25); // завершаем большой цикл.  в зависимости от параметра line определяем количество необходимых строк, т.е сколько раз цикл будет выполнять код
